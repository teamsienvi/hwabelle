    ...
    return (
        <div className="h-screen flex flex-col overflow-hidden bg-background">
            ...
            <div className="bg-amber-500 text-white flex-shrink-0 z-50">
                <div className="container flex items-center justify-between py-2 px-4 text-sm">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="hidden sm:inline">— Preview the complete subscriber experience</span>
                    </div>
                    <button
                        onClick={resetAll}
            ...
            <div className="bg-amber-50 dark:bg-amber-950/30 border-b border-amber-200 dark:border-amber-800/50 flex-shrink-0 z-40">
                <div className="container flex items-center gap-3 py-2 px-4 text-xs flex-wrap">
                    <span className="text-amber-800 dark:text-amber-300 font-medium mr-1">SIMULATE:</span>
                    ...
                    {[
                        ...
                        <button
                            key={opt.months}
                            onClick={() => { setSubState("active"); setMonthsActive(opt.months); }}
                            className={`px-3 py-1 rounded-md font-medium transition-colors ${subState === "active" && monthsActive === opt.months
                                ? "bg-emerald-600 text-white"
                                : "bg-white dark:bg-amber-900/40 text-amber-800 dark:text-amber-300 border border-amber-200 dark:border-amber-700 hover:bg-emerald-50 dark:hover:bg-emerald-900/30"
                                }`}
                        >
                            {opt.label}
                        </button>
                    ...
                    <div className="w-px h-5 bg-amber-300 dark:bg-amber-700 mx-1" />

                    {/* Expire */}
                    <button
                        onClick={() => setSubState("expired")}
                        className={`px-3 py-1 rounded-md font-medium transition-colors ${subState === "expired"
                            ? "bg-red-600 text-white"
                            : "bg-white dark:bg-amber-900/40 text-red-600 border border-red-200 dark:border-red-700 hover:bg-red-50 dark:hover:bg-red-900/30"
                            }`}
                    >
                        Expire
                    </button>
                    ...
                    <div className="ml-auto flex items-center gap-2">
                        <div className={`w-2 h-2 rounded-full ${subState === "active" ? "bg-emerald-500" : "bg-red-500"}`} />
                        <span className={`font-medium ${subState === "active" ? "text-emerald-700 dark:text-emerald-400" : "text-red-600 dark:text-red-400"}`}>
                            {subState === "active" ? `Active — ${monthsActive} mo` : "Expired"}
                        </span>
                    </div>
                </div>
            </div>
                ...
                <div className="border-b border-divider/50 backdrop-blur-md z-10 flex-shrink-0" style={{ background: "hsla(var(--background), 0.85)" }}>
                    <div className="container max-w-3xl py-4 px-4 flex items-center gap-3">
                        <div className="relative">
                            ...
                            <div className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-background ${subState === "active" ? "bg-emerald-500" : "bg-red-400"}`} />
                        </div>
                        <div>
                            <h1 className="font-serif text-lg leading-tight">Floral Designer</h1>
                            <p className="text-xs text-muted-foreground">
                                {subState === "active" ? "Your botanical companion" : "Subscription required"}
                            </p>
                        </div>
                    </div>
                </div>
